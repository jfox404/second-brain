...
A VS Code + Foam-based second brain vault with three layers: (1) a frictionless capture surface (CLI + daily notes), (2) an AI-assisted review pass that enriches captures in-place, and (3) a lightweight PARA-based organization layer enforced by the AI, not by manual filing.
...
5. As a user, I want to invoke a review skill that processes unprocessed captures in today's daily note, so that I can enrich my notes without manual work.
6. As a user, I want the review skill to suggest wikilinks between my captures and existing vault notes, so that the graph becomes more connected.
7. As a user, I want the review skill to assign PARA type tags (`#project/x`, `#area/x`, `#resource/x`, `#archive/x`) to captures, so that they're categorized without manual decisions.
8. As a user, I want the review skill to extract action items marked `[action:]` from my captures and compile them into project task sections and a central `tasks.md`, so that I can plan from both per-project and aggregate views.
9. As a user, I want the review skill to promote noteworthy captures to standalone files under the appropriate type folder, so that durable ideas have their own permanent home.
10. As a user, I want the review skill, when it encounters a URL in a capture, to fetch the linked content and synthesize a promoted resource note, so that article/book highlights are captured without extra effort.
11. As a user, I want the review skill to mark processed captures with a ✅ prefix so that I can see what's been reviewed at a glance.
...
17. As a user, I want the review skill to maintain a `## Tasks` section inside each project note, so that project-level task visibility is preserved.
...
21. As a user, I want the project creation skill to accept arguments so it can be invoked non-interactively (e.g., by the review pass), so that AI can create projects without blocking on input.
22. As a user, I want the review pass to flag captures tagged with a new `#project/x` that has no corresponding project note, and suggest creating one, so that project discovery is automatic.
...
31. As a user, I want resource notes to be creatable both by the AI review pass (from URL captures) and manually via a skill/template, so that all reference material has a consistent structure.
...
- **Review marker**: Processed captures get a `✅` prefix: `- ✅ [HH:MM] thought`. The review pass only processes captures without the marker.
